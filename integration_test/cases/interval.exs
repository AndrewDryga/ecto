defmodule Ecto.Integration.IntervalTest do
  ...
  test "date_add with week" do
    dec = Decimal.new(3)
    assert [{2014, 1, 22}] =
           TestRepo.all(from p in Post, select: date_add(p.posted, 3, "week"))
    assert [{2014, 1, 22}] =
  ...
  test "date_add with day" do
    dec = Decimal.new(5)
    assert [{2014, 1, 6}] =
           TestRepo.all(from p in Post, select: date_add(p.posted, 5, "day"))
    assert [{2014, 1, 6}] =
  ...
  test "date_add with dynamic" do
    posted = @posted
    assert [{2015, 1, 1}]  =
           TestRepo.all(from p in Post, select: date_add(^posted, ^1, ^"year"))
    assert [{2014, 4, 1}]  =
           TestRepo.all(from p in Post, select: date_add(^posted, ^3, ^"month"))
    assert [{2014, 1, 22}] =
           TestRepo.all(from p in Post, select: date_add(^posted, ^3, ^"week"))
    assert [{2014, 1, 6}]  =
           TestRepo.all(from p in Post, select: date_add(^posted, ^5, ^"day"))
    assert [{2014, 1, 3}]  =
  ...
  test "from_now" do
    ...
    dec = Decimal.new(5)
    assert [{{y, _, _}, _}] =
           TestRepo.all(from p in Post, select: from_now(5, "year"))
    assert y > current
    assert [{{y, _, _}, _}] =
           TestRepo.all(from p in Post, select: from_now(5.0, "year"))
    assert y > current
    assert [{{y, _, _}, _}] =
           TestRepo.all(from p in Post, select: from_now(^5, "year"))
    assert y > current
    ...
    assert y > current
    assert [{{y, _, _}, _}] =
           TestRepo.all(from p in Post, select: from_now(^dec, "year"))
    assert y > current
  end
  ...
  test "ago" do
    ...
    dec = Decimal.new(5)
    assert [{{y, _, _}, _}] =
           TestRepo.all(from p in Post, select: ago(5, "year"))
    assert y < current
    assert [{{y, _, _}, _}] =
           TestRepo.all(from p in Post, select: ago(5.0, "year"))
    assert y < current
    assert [{{y, _, _}, _}] =
           TestRepo.all(from p in Post, select: ago(^5, "year"))
    assert y < current
    ...
    assert y < current
    assert [{{y, _, _}, _}] =
           TestRepo.all(from p in Post, select: ago(^dec, "year"))
    assert y < current
  end
end
